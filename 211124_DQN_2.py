...
# Soil layers
soil1 = Polygon(np.array([[6,4],[30,20],[54,20]]))
soil2 = Polygon(np.array([[-18,0],[0,0],[6,4],[54,20],[78,20],[-18,-12]]))
soil3 = Polygon(np.array([[-18,-12],[78,20],[78,-12]]))
...
step_x0 = 0.25 # x0 step size
xmin = -10
xmax = 60
ymin = -10
ymax = h
alphamin = -(math.pi/4-phi_2/2)
...
# Input
if mode != 'random':
    f = open('DQN_Stat_Res_2.csv','w')
else:
    f = open('DQN_Rand_Res_2.csv','w')
...
#        plt.plot(xslip_his[i], yslip_his[i], linewidth=0.5, color=(col_r,col_g,0,0.5))
    plt.plot(xslip_his[i], yslip_his[i], linewidth=0.5, color=(col_r,col_g,0,0.5))
# Plot soil layers
plt.plot([6,54],[4,20], color = 'gray', linewidth = 1)
plt.plot([-20,80],[-12.667,20], color = 'gray', linewidth = 1)
# Plot critical slip surface
plt.plot(xslip_his[crit_slope], yslip_his[crit_slope], linewidth=0.5, color=(0,0,0,1))
plt.text(38,42-6,'Critical slip surface', color=(0,0,0),family='Arial') 
plt.text(38,42-9,'$\mu$ = ' + str(round(mu_his[crit_slope],3)), color=(0,0,0),family='Arial') 
plt.text(38,42-12,'$F_s$ = ' + str(round(1/mu_his[crit_slope],3)), color=(0,0,0),family='Arial') 
plt.text(38,42-15,'$x_0$ = ' + str(x0_his[crit_slope]) + ' m', color=(0,0,0),family='Arial') 
plt.text(38,42-18,'$n_s$ = ' + str(len(xslip_his[crit_slope])-1), color=(0,0,0),family='Arial') 
# Plot slices
...
# Plot slope
plt.plot([xmin,0,b,100], [0,0,h,h], color = 'black', linewidth=1)
# Color bar
for cb in np.arange(9.5,36.6,0.1):
    y_s = (cb-9)/(37-9)
#    y_s = y_s**3
    col_r = min(2*y_s,1)
    col_g = min(2*(1-y_s),1)
    plt.plot([-7.5,-6.5],[cb,cb], color = (col_r,col_g,0,0.5), linewidth = 4)
plt.plot([-8,-6,-6,-8,-8],[9,9,37,37,9], color = 'black', linewidth = 1)
for cb_thick in np.arange(9,41,4):
    plt.plot([-6,-5.4],[cb_thick,cb_thick], color = 'black', linewidth = 1)
    if cb_thick !=10:
        mu_y = (mu_max-mu_min)/(37-9)*(cb_thick-9)+mu_min
        plt.text(-5.0,cb_thick-0.3, str(round(mu_y,3)), color='black', fontsize=7,family='Arial')
    # else:
        # plt.text(-15.0,cb_thick-0.3, '< ' + str(round(mu_min,3)), color='black', fontsize=7,family='Arial')
# Save picture to file
if mode != 'random':
    plt.savefig('DQN_Stat_Slip_2.pdf')
else:
    plt.savefig('DQN_Rand_Slip_2.pdf')    
plt.show()
...
font = font_manager.FontProperties(family='Arial',
                                   size=10)
plt.legend(['Utilisation','Moving average'],fancybox=False,edgecolor='0',prop=font,loc='upper right',facecolor='white', framealpha=1)
if mode != 'random':
    plt.savefig('DQN_Stat_Util_2.pdf')
else:
    plt.savefig('DQN_Rand_Util_2.pdf')    
plt.show()
...
# if mode != 'random':
#     plt.savefig('DQN_Stat_Util_Val_2.pdf')
# else:
#     plt.savefig('DQN_Rand_Util_Val_2.pdf')    
# plt.show()
...
font = font_manager.FontProperties(family='Arial',
                                   size=10)
plt.legend(['Total reward','Moving average'],fancybox=False,edgecolor='0',prop=font,loc='upper right',facecolor='white', framealpha=1)
if mode != 'random':
    plt.savefig('DQN_Stat_Rew_2.pdf')
else:
    plt.savefig('DQN_Rand_Rew_2.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Loss_2.pdf')
else:
    plt.savefig('DQN_Rand_Loss_2.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Slices_2.pdf')
else:
    plt.savefig('DQN_Rand_Slices_2.pdf')    
plt.show()
