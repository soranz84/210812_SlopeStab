...
# Slope geometry 
b = 20 # Slope base width
h = 10

# Soil layers
soil1 = Polygon(np.array([[0,0],[20,4],[24,6],[24,30],[40,10],[20,10]]))
soil2 = Polygon(np.array([[10,2],[20,4],[24,6],[24,30],[30,-1],[22,-1]]))
soil3 = Polygon(np.array([[-5,0],[0,0],[10,2],[22,-1],[30,-1],[30,-5],[-5,-5]]))
...
# Input
if mode != 'random':
    f = open('DQN_Stat_Res_3.csv','w')
else:
    f = open('DQN_Rand_Res_3.csv','w')
...
# Plot soil layers
plt.plot([0,10,20,24,xmax],[0,2,4,6,6], color = 'gray', linewidth = 1)
plt.plot([10,22,xmax],[2,-1,-1], color = 'gray', linewidth = 1)
# Plot critical slip surface
plt.plot(xslip_his[crit_slope], yslip_his[crit_slope], linewidth=0.5, color=(0,0,0,1))
plt.text(0.9*b,2.0*h-2,'Critical slip surface', color=(0,0,0)) 
plt.text(0.9*b,2.0*h-3.5,'$\mu$ = ' + str(round(mu_his[crit_slope],3)), color=(0,0,0)) 
plt.text(0.9*b,2.0*h-5,'$F_s$ = ' + str(round(1/mu_his[crit_slope],3)), color=(0,0,0)) 
plt.text(0.9*b,2.0*h-6.5,'$x_0$ = ' + str(x0_his[crit_slope]) + ' m', color=(0,0,0)) 
plt.text(0.9*b,2.0*h-8,'$n_s$ = ' + str(len(xslip_his[crit_slope])-1), color=(0,0,0)) 
# Plot slices
...
# Plot slope
plt.plot([-b/2,0,b,xmax], [0,0,h,h], color = 'black', linewidth=1)
# Color bar
for cb in np.arange(10.2,18.8,0.1):
    y_s = (cb-10)/(19-10)
#    y_s = y_s**3
    col_r = min(2*y_s,1)
    col_g = min(2*(1-y_s),1)
    plt.plot([-3.8,-2.2],[cb,cb], color = (col_r,col_g,0,0.5), linewidth = 4)
plt.plot([-4,-2,-2,-4,-4],[10,10,19,19,10], color = 'black', linewidth = 1)
for cb_thick in np.arange(10,20,1):
    plt.plot([-2,-1.8],[cb_thick,cb_thick], color = 'black', linewidth = 1)
    if cb_thick != 20 and cb_thick !=10:
        mu_y = (mu_max-mu_min)/(19-10)*(cb_thick-10)+mu_min
        plt.text(-1.5,cb_thick-0.3, str(round(mu_y,3)), color='black', fontsize=7)
    if cb_thick == 10:
        plt.text(-1.5,cb_thick-0.3, '< ' + str(round(mu_min,3)), color='black', fontsize=7)
# Save picture to file
if mode != 'random':
    plt.savefig('DQN_Stat_Slip_3.pdf')
else:
    plt.savefig('DQN_Rand_Slip_3.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Util_3.pdf')
else:
    plt.savefig('DQN_Rand_Util_3.pdf')    
plt.show()
...
plt.legend(['Utilisation','Moving average'],fancybox=False,edgecolor='0',prop=font,loc='lower right',facecolor='white', framealpha=1)
if mode != 'random':
    plt.savefig('DQN_Stat_Util_Val_3.pdf')
else:
    plt.savefig('DQN_Rand_Util_Val_3.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Rew_3.pdf')
else:
    plt.savefig('DQN_Rand_Rew_3.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Loss_3.pdf')
else:
    plt.savefig('DQN_Rand_Loss_3.pdf')    
plt.show()
...
if mode != 'random':
    plt.savefig('DQN_Stat_Slices_3.pdf')
else:
    plt.savefig('DQN_Rand_Slices_3.pdf')    
plt.show()
